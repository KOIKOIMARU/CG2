...
void main(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    if (0 <= freeListIndex && freeListIndex < kMaxParticles)
    {
        ...
        float2 randomScale = lerp(
            gEmitter.scaleMin,
            gEmitter.scaleMax,
            generator.Generate3d().xy);
        float3 randomDirection = generator.Generate3d() * 2.0f - 1.0f;
        float3 emissionDirection = normalize(
            gEmitter.direction + randomDirection * gEmitter.spread);
        float speed = lerp(
            gEmitter.speedMin,
            gEmitter.speedMax,
        ...
        gParticles[particleIndex].lifeTime = lerp(
            gEmitter.lifeTimeMin,
            gEmitter.lifeTimeMax,
            generator.Generate1d());
